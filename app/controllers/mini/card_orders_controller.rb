# encoding: utf-8
class Mini::CardOrdersController < Mini::BaseController
  layout "mini/user"

  def new
    @cards = Mini::Giftcard.items(request, {id: Mini::Giftcard::DEFAULT_ID})["data"]["items"]
  end

  # 自己购买
  def my_list
    @title = "自己购买"
    @my = Mini::Giftcard.list(request, {type: 1, page: (params[:page] || 1), pagesize: 10})
  ...
  def received_list
    @title = "好友赠送"
    @received = Mini::Giftcard.list(request, {type: 2, page: (params[:page] || 1), pagesize: 10})
    respond_to do |format|
      format.html{}
  ...
  def create
    ...
    # TODO 上线前，修改为正式地址
    @notify_url = 'http://111.207.166.195/mini/payment/notify_giftcard'
    @time_val = Time.now
  ...
  def check_status
    ...
    result = Mini::UserApi.latest_giftcard(request, {"giftcardid" => params["giftcardid"], "timestamp" => params["timestamp"]})
