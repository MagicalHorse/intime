# encoding: utf-8
class Mini::CardsController < Mini::BaseController
  ...
  def recharge
    ...
    if current_user.isbindcard
      # API_NEED: 充值礼品卡接口
      @result = Mini::Giftcard.recharge(request, charge_no: @charge_no)
      if @result[:isSuccessful]
        # 置空欲充值卡号
        current_user.will_charge_no = nil
        # API_NEED: 根据礼品卡号，获取礼品卡相关信息
        @card = Mini::Giftcard.detail(request, charge_no: @charge_no)["data"] || {}
      else
        @card = {}
  ...
  def gift_page
    ...
    @charge_no = params[:charge_no]
    # API_NEED: 根据礼品卡号，获取礼品卡相关信息
    @card = Mini::Giftcard.transfer_detail(request, charge_no: @charge_no)["data"] || {}
  ...
  def give_page
    ...
    # API_NEED: 根据礼品卡号，获取礼品卡相关信息
    @card = Mini::Giftcard.detail(request, charge_no: @charge_no)["data"]
    @card = (Mini::Giftcard.transfer_detail(request, charge_no: @charge_no)["data"] || {}) if @card.blank?
    current_user.other_phone = @card[:phone]
  end
  ...
  def give
    ...
    if @notice
      return redirect_to "#{give_page_mini_cards_path(charge_no: @charge_no, phone: params[:phone], comment: params[:comment], from: params[:from])}", notice: @notice
    else
      # API_NEED: 赠送礼品卡接口
  ...
  def refuse
    # API_NEED: 拒收并退回礼品卡
    Mini::Giftcard.refuse(request, charge_no: params[:charge_no])
    return redirect_to gift_page_mini_cards_path(charge_no: params[:charge_no], _: Time.now.to_i)
  end
