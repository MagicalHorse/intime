# encoding: utf-8
class Mini::Store::CombosController < Mini::Store::BaseController
  ...
  def destroy
    @status = Mini::Combo.delete(request, {:id => params[:id]})
  ...
  def update
    ...
    if @combo.remote_id.present?
      @remote_combo = Mini::Combo.update(request, @combo.api_attrs.merge({:id => @combo.remote_id}))
    else
      @remote_combo = Mini::Combo.create(request, @combo.api_attrs)
    end
  end

  def add_img
    @combo = ::Combo.find(params[:id])
    @image = Mini::Combo.upload_img(request, {:image => params[:img], :image_type => 15})
