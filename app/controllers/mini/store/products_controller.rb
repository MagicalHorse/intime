# encoding: utf-8
class Mini::Store::ProductsController < Mini::Store::BaseController
  ...
  def index
    ...
    @search = Mini::Product.list(request, page: params[:page], pagesize: params[:per_page] || 10)
  ...
  def show
    @title = "商品详情"
    @product = Mini::Product.find(request, {id: params[:id]})[:data]
  end
  ...
  def create
    ...
    product = Mini::Product.create(request, {
      image: params["image"],
    ...
    if product["isSuccessful"]
      ...
      redirect_to new_mini_store_combo_path(:combo_id => @combo.try(:id), t: Time.now.to_i)
    else
      redirect_to new_mini_store_product_path(:combo_id => @combo.try(:id))
    end
  end

  def update
    product = Mini::Product.update(request, {
      id: params[:id],
      image: params["image"],
      brand_id: params["brand_id"],
      sales_code: params["coding_id"],
    ...
    if product["isSuccessful"]
      redirect_to mini_store_products_path
    else
      redirect_to edit_mini_store_product_path(params[:id])
    end
  end
  ...
  def add_to_combo
    ...
    product = params[:product_type] == "2" ? Mini::Product.find(request, {:id => params[:id]}) : ::Product.fetch_product(params[:id])
    ...
    redirect_to new_mini_store_combo_path(:combo_id => @combo.id, t: Time.now.to_i)
  end
  ...
  def product_relation_data
    @brands = Brand.es_search
    # @categories = Mini::ProductCategory.list(request)["data"]["items"]
    @categories = Tag.es_search(per_page: 200000)[:data]
    @codings = Mini::ProductCoding.list(request)["data"]["items"]
  end
