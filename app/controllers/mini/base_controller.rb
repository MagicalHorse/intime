# encoding: utf-8
class Mini::BaseController < ApplicationController
  layout 'mini'
  before_filter :wx_auth!
  helper_method :current_user

  rescue_from Mini::Unauthorized do
    redirect_to(URI::HTTPS.build([nil, "open.weixin.qq.com", nil, "/connect/oauth2/authorize", {appid: Settings.wx.appid, redirect_uri: URI.escape("http://#{Settings.wx.backdomain}/mini/auth"), response_type: 'code', scope: 'snsapi_base', state: "STATE"}.to_param, 'wechat_redirect']).to_s)
    session[:back_url] = request.url
  end

  def backurl
    session[:back_url]||mini_cards_path
  end
  ...
  def wx_auth!
    ...
    if request.host != "114.215.179.76" || request.headers["HTTP_USER_AGENT"].include?("Firefox") || request.headers["HTTP_USER_AGENT"].include?("Chromium") || request.headers["HTTP_USER_AGENT"].include?("Chrome")
      ...
      raise Mini::Unauthorized if session[:user_token].blank? || cookies[:user_access_token].blank?
    end
  end
  ...
  def generate_sms phone
    ...
    Mini::Sms.send(request, {phone: phone, text: "验证码：#{current_user.sms_code}，请填写验证码并完成操作。【请勿向任何人提供您收到的短信验证码】【迷你银】"})
  end
  ...
  def user_account_info
    ...
    data = Mini::Giftcard.my(request)[:data]
    if data.present?
