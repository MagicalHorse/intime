...
class Mini::OrdersController < Mini::BaseController
  def index
    @search = Mini::Order.my(request, page: params[:page], pagesize: params[:per_page] || 10)
  ...
  def new
    ...
    if params[:product_id].present?
      ...
      @products = Mini::Order.new(request, id: params[:combo_id])[:data][:items]
      @order = Mini::Order::computeamount(request, combo_id: params["combo_id"], quantity: 1)[:data]
    end
  ...
  def show
    @order = Mini::Order.detail(request, {orderno: params["id"]})["data"]
    @title = "订单详情"
  ...
  def payments
    ...
    # TODO 上线前，修改为正式地址
    @notify_url = 'http://111.207.166.195/mini/payment/notify'
    @time_val = Time.now
